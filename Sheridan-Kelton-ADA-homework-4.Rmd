---
title: "Sheridan-Kelton-ADA-homework-4"
...
#United States, Spain, United Kingdom, Italy, Russia, France, Germany, Brazil, Turkey, Iran

daily <- filter(d, variable == "daily_confirmed")
#United States, France, China, Ecuador, Russia, Brazil, Spain, United Kingdom, Germany, Italy
...
library(MASS)
full <- lm(data = overall, rate ~ density + population + gdp_capita + income)
full
(s <- stepAIC(full, scope = . ~ ., direction = "both"))
summary(s)

#minimal model 
min <- lm(data = overall, rate ~ 1)
min
(s2 <- stepAIC(min, scope = ~ . + density + population + gdp_capita +
                income, direction = "both"))
summary(s2)

library(AICcmodavg)
aictab(list(full, min), c("full", "min"))
...
#Challenge 8
last <- filter(cv_data, variable == "daily_confirmed", population > 1000000, rate > 0)
last
library(lme4)
library(nlme)
head(last)
m1<- lmer(data = last, log(rate) ~ log(density) + log(gdp_capita) + soc_dist + mov_rest +
